...
`build-poster.mjs` 使用 `@oai/artifact-tool` 创建可编辑草稿，读取 `content.json`、`forms/links.json`、`qr-code.png`、`public/images/student-collaboration.png` 和 `public/brand/nexabio-logo-dark-v1.png`。需在可用的 Artifact Tool 环境执行 `node poster/build-poster.mjs`；若包在其他目录，用 `ARTIFACT_NODE_MODULES` 指定对应 `node_modules`。草稿写入 `.poster-build/`，不会覆盖已审核的 PPTX。
