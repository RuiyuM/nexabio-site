...
一页 US Letter 竖版（8.5 × 11 英寸），沿用网站的深绿、橙色与白色。文字和色块是 PowerPoint 原生对象；二维码是独立图片。当前是学生意向登记草稿，未承诺名额、资助、录取或最终日期。
...
`build-poster.mjs` 使用 `@oai/artifact-tool` 创建可编辑草稿，读取 `content.json`、`forms/links.json` 和 `qr-code.png`。需在可用的 Artifact Tool 环境执行 `node poster/build-poster.mjs`；若包在其他目录，用 `ARTIFACT_NODE_MODULES` 指定对应 `node_modules`。草稿写入 `.poster-build/`，不会覆盖已审核的 PPTX。
